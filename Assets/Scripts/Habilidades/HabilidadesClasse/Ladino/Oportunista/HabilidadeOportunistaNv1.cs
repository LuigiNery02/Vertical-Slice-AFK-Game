...
[CreateAssetMenu(menuName = "Habilidades/Passiva/Classe/Ladino/Oportunista/Nv1")]
public class HabilidadeOportunistaNv1 : HabilidadePassiva
{
    [Header("Configurações da Habilidade")]
    [SerializeField]
    private float multiplicadorDano = 0.2f;
    ...
    private IEnumerator MonitorarCondicao(IAPersonagemBase personagem, DadosHabilidadePassiva dados)
    {
        while (true)
        {
            ...
            if (alvo != null && alvo.VerificarEfeitoNegativo())
            {
                if (!dados.bonusAplicado)
                {
